...
class SurveyorMojo : AbstractMojo() {
    ...
    override fun execute() {
        val agentJar = pluginArtifacts
            .firstOrNull { it.groupId == "com.antwerkz" && it.artifactId == "surveyor-agent" }
            ?.file
            ?: throw org.apache.maven.plugin.MojoExecutionException(
                "surveyor-agent artifact not found in plugin artifacts. " +
                "Ensure the surveyor-maven-plugin is correctly configured."
            )
        ...
        log.info("Surveyor agent configured: $javaAgentArg")
    }
}
