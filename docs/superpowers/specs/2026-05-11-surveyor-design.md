# Surveyor — Design Document
Date: 2026-05-11

## Overview

Surveyor is a JVM agent + Maven plugin that instruments every non-synthetic method
...
```
surveyor/                          ← parent POM (com.antwerkz:surveyor, packaging=pom)
├── surveyor-agent/                ← com.antwerkz:surveyor-agent
│   └── pom.xml                    (jar → shaded fat JAR via maven-shade-plugin)
└── surveyor-maven-plugin/         ← com.antwerkz:surveyor-maven-plugin
...
## Agent Module (`surveyor-agent`)

### Entry point

`com.antwerkz.surveyor.SurveyorAgent` is declared as `Premain-Class` in the JAR manifest.
...
runner), all spans fall under a single `surveyor-run` root span.
...
`com.antwerkz.surveyor.shaded.*` via `maven-shade-plugin` to prevent version conflicts
with the target application's own classpath.

---

## Maven Plugin Module (`surveyor-maven-plugin`)

### Lifecycle binding

Binds to the `initialize` phase. Locates the `surveyor-agent` JAR from the plugin's
...
| `outputDirectory`| `${project.build.directory}/surveyor`      | No       | Directory for output trace files                     |
...
```
-javaagent:/path/to/surveyor-agent.jar=packages=com.example,outputDir=/path/to/dir[,endpoint=http://...,captureArgs=true,maxArgLength=256]
```

### Activation

The plugin is added to the user's project under a Maven profile of their choosing.
Surveyor does not ship a fixed profile name.
...
Test execution
  └── JUnit/TestNG test method begins
        ...
        └── Test method ends → endSpan (root)
              └── FileExporter flushes → target/surveyor/<TestClass#testMethod>.json
...
Surveyor must never fail the build.
...
**Guiding principle: Surveyor must never alter the outcome of a test run.**

---

## Testing Strategy

### Unit tests — `surveyor-agent`
...
### Unit tests — `surveyor-maven-plugin`
...
### Integration tests — `surveyor-maven-plugin/src/it/`
