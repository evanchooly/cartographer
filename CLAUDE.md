# Cartographer — Project Instructions

## Source of Truth

`docs/superpowers/specs/2026-05-11-cartographer-design.md` is the authoritative specification for this project's behavior and design.
...
Cartographer is a JVM agent + Maven plugin that instruments non-synthetic methods (including constructors) in classes
under a configured package with OpenTelemetry span decorations during test runs. See `docs/superpowers/specs/2026-05-11-cartographer-design.md` for full details.
...
- Output goes to `target/cartographer/` by default in a format Jaeger can ingest
...
- `cartographer-agent` — the Java agent (bytecode instrumentation, OTel SDK setup, file exporter)
- `cartographer-maven-plugin` — the Maven plugin that injects the agent into the test JVM
